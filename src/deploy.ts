/**
 * Deploy midnight-project contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy midnight-project to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  console.log('  Waiting 5 seconds for indexer sync before setup...');
  await new Promise(resolve => setTimeout(resolve, 5000));
  console.log('  Setting up the Inheritance contract with initial values...');
  
  const hiddenBeneficiaryHash = new Uint8Array(32);
  hiddenBeneficiaryHash.fill(1); // Dummy hash

  const setupTx = await deployed.callTx.setup(
    86400n, // timeout: 1 day
    hiddenBeneficiaryHash, // hiddenBeneficiaryHash (32 bytes)
    BigInt(Math.floor(Date.now() / 1000)) // currentTime
  );
  console.log(`  ✅ Setup circuit executed! TX Hash: ${setupTx.public.txHash}`);
